...
Процедура ДобавитьШаги(Дерево,Тип,ПредставлениеТеста,ОписаниеШага,ИмяФайла,ТаблицаПеревода,СтрТаблицаИзвестныхStepDefinition)
	...
	Если ТаблицаПеревода <> Неопределено Тогда
		...
		Если СтрокаТаблицаПеревода <> Неопределено Тогда
			...
			КонецЕсли;
		Иначе
			ТексСообщения = НСтр("ru = 'Не найден перевод для шага: %1'");
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон(ТексСообщения, ПредставлениеТеста);
			Сообщение.Сообщить();
...
Процедура ДеревоШаговВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ТекстСообщения = НСтр("ru = 'Добавлен шаг: %1'");
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = СтрШаблон(ТекстСообщения, Элемент.ТекущиеДанные.ПредставлениеТеста);
	Сообщение.Сообщить();
...
&НаСервере
Функция СоздатьFeatureReader(КаталогИнструментов, ДвДанныеvbFeatureReader, ИмяВременногоФайла)
	...
	Если Не ФайлvbFeatureReader.Существует() Тогда
		...
		ДвДанныеvbFeatureReader.Записать(ИмяВременногоФайла);
		FeatureReader = ВнешниеОбработки.Создать(ИмяВременногоФайла, Ложь);
	Иначе
...
&НаСервере
Процедура ДобавитьЗначенияВТаблицуДляПериодаРекурсивно(ТаблицаДляПереводаИзвестныхШагов, Дерево)
	...
	Для Каждого СтрокаДерева Из Дерево.Строки Цикл
		...
		ДобавитьЗначенияВТаблицуДляПериодаРекурсивно(ТаблицаДляПереводаИзвестныхШагов,СтрокаДерева);
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуДляПеревода(ТаблицаДляПереводаИзвестныхШагов,Дерево)
	...
	ДобавитьЗначенияВТаблицуДляПериодаРекурсивно(ТаблицаДляПереводаИзвестныхШагов,Дерево);

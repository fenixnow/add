...
Перем РазницаВМиллисекундахМеждуЮниксИНачалЭпохи;
	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		Если СтрСтроки.Тип = "Фича" Тогда
			...
			Если ОтчетВРежимеУФ Тогда
				СтруктураДляФайлаXML = Новый Структура;
				СтруктураДляФайлаXML.Вставить("РеальноеИмяФайла",РеальноеИмяФайла);
			...
			Если ОтчетВРежимеУФ Тогда
				...
				СтруктураДляФайлаXML.Вставить("ФайлXMLДвоичныеДанные",ДвоичныеДанные);
				ПараметрыОтчетаУФ.МассивXMLОтчетаAllure.Добавить(СтруктураДляФайлаXML);
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") Или (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Сценарий.start = РезультатПрохожденияТестовСценария.ВремяНачала - РазницаВМиллисекундахМеждуЮниксИНачалЭпохи;
				Сценарий.stop  = РезультатПрохожденияТестовСценария.ВремяОкончания - РазницаВМиллисекундахМеждуЮниксИНачалЭпохи;
				...
				Иначе
					Для каждого СтрРезультатПрохожденияТестовШагов Из РезультатПрохожденияТестовСценария.РезультатПрохожденияТестовШагов Цикл
						...
						Если СтрРезультатПрохожденияТестовШагов.ВремяНачала <> Неопределено Тогда
							Шаг.start = СтрРезультатПрохожденияТестовШагов.ВремяНачала - РазницаВМиллисекундахМеждуЮниксИНачалЭпохи;;
							Шаг.stop  = СтрРезультатПрохожденияТестовШагов.ВремяОкончания - РазницаВМиллисекундахМеждуЮниксИНачалЭпохи;;
						КонецЕсли;
...
Процедура СформироватьОтчетАллюр(Знач СтруктураОФ) Экспорт
	...
	Если СтруктураОФ.Свойство("ЭтоОФ") Тогда
		...
		Если Не Файл.Существует() Тогда
			СделатьСообщение("Отчет Allure не сформирован. Не существует каталог для вывода отчета.");
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Аллюр2.СформироватьОтчетАллюр2(СтруктураОФ, ДеревоТестов, ИмяТекущейСборки,
...
Функция Определить_classname(ДопПараметры)
	...
	ИмяКласса = ДопПараметры.ТекущаяФичаИмя;

	Если ДобавлятьКИмениСценарияУсловияВыгрузки Тогда
		Если ДопПараметры.ТекУровеньКаталога >= 2 Тогда
			ИмяКласса = ДопПараметры.УровниКаталогов[0] + "." + ДопПараметры.ТекущаяФичаИмя;
		КонецЕсли;
	Иначе
		Если ДопПараметры.ТекУровеньКаталога >= 2 Тогда
			ИмяКласса = ДопПараметры.УровниКаталогов[0] + "." + ДопПараметры.ТекущаяФичаИмя;
				...
				Для Каждого Шаг Из Сценарий.МассивШагов Цикл
					...
					ИначеЕсли Шаг.Статус = "Pending" Тогда
						...
						Иначе
							...
							ЕстьНеРеализованныйШаг = Истина;
						КонецЕсли;
					Иначе
						Если ЕстьУпавшийШаг или ЕстьНеРеализованныйШаг Тогда
							ЗаписьJSON.ЗаписатьЗначение("skipped");
						Иначе
			...
			Иначе
				...
				Иначе
					Для каждого СтрРезультатПрохожденияТестовШагов Из РезультатПрохожденияТестовСценария.РезультатПрохожденияТестовШагов Цикл
						...
						Если СтрРезультатПрохожденияТестовШагов.ВремяНачала <> Неопределено Тогда
							ДанныеШага.Вставить("start",СтрРезультатПрохожденияТестовШагов.ВремяНачала - РазницаВМиллисекундахМеждуЮниксИНачалЭпохи);
							ДанныеШага.Вставить("stop",СтрРезультатПрохожденияТестовШагов.ВремяОкончания - РазницаВМиллисекундахМеждуЮниксИНачалЭпохи);
						КонецЕсли;
						...
						Если РезультатПрохожденияТестовСценария.Свойство("МассивСкриншотов") И СтрРезультатПрохожденияТестовШагов.Статус = "Failed" Тогда
							...
							Для каждого СтруктураСкриншот Из РезультатПрохожденияТестовСценария.МассивСкриншотов Цикл
								...
								СтруктураРисунка = Новый Структура("mime_type, data, binarydata", "image/png", СтрокаРисунка, СтруктураСкриншот.ДвоичныеДанные);
...
Функция ФорматДСО(ДопСообщениеОшибки)
	...
	Возврат ?(ПустаяСтрока(ДопСообщениеОшибки), "", Символы.ПС + ДопСообщениеОшибки);
...
Процедура ПроверитьРавенство(Значение1, Значение2, ДопСообщениеОшибки = "") Экспорт
	
	Если Значение1 <> Значение2 Тогда
		СообщениеОшибки = "Сравниваемые значения ("+Значение1+"; "+Значение2+") не равны, а хотели, чтобы были равны." + ФорматДСО(ДопСообщениеОшибки);
		ВызватьОшибкуПроверки(СообщениеОшибки);
	КонецЕсли;
...
Процедура ПроверитьБольше(Значение1, Значение2, ДопСообщениеОшибки = "") Экспорт
	
	Если Значение1 <= Значение2 Тогда
		СообщениеОшибки = "Первый параметр ("+Значение1+") меньше или равен второму ("+Значение2+") а хотели, чтобы был больше." + ФорматДСО(ДопСообщениеОшибки);
		ВызватьОшибкуПроверки(СообщениеОшибки);
	КонецЕсли;
...
Процедура ПроверитьМеньшеИлиРавно(Значение1, Значение2, СообщениеОбОшибке = "") Экспорт
	
	Если Значение1 > Значение2 Тогда
		СообщениеОшибки = СтрШаблон("Значение <%1> больше, чем <%2>, а ожидалось меньше или равно %3", Значение1, Значение2, ФорматДСО(СообщениеОбОшибке));
		ВызватьОшибкуПроверки(СообщениеОшибки)
	КонецЕсли;
...
//TODO исправить дублирование метода СравнитьТаблицы в плагине УтвержденияПроверкаТаблиц
...
Функция ПрочитатьСтруктуруИзJSONФайла(ИмяФайлаVBParams, ПараметрыУФ = Неопределено) Экспорт
	...
	ВремНастройкиСоответствие = ПрочитатьНастройкиИзФайлаJSon(ИмяФайла);
	Возврат ВремНастройкиСоответствие;
...
Процедура СоздатьКолонкиДерева(Дерево) Экспорт
	...
	Дерево.Колонки.Добавить("Сниппет");
	Дерево.Колонки.Добавить("АдресСниппета");
	Дерево.Колонки.Добавить("ЭтоЧужойСниппет");
...
Процедура ДобавитьШагВМассивТестов(МассивТестов,Сниппет,ИмяПроцедуры,ПредставлениеТеста = Неопределено,ОписаниеШага = Неопределено,ТипШагаДляОписания = Неопределено,ТипШагаВДереве = Неопределено) Экспорт
	
	Структура = Новый Структура;

	Структура.Вставить("Сниппет",Сниппет);
...
РазницаВМиллисекундахМеждуЮниксИНачалЭпохи = 62135596800000;
